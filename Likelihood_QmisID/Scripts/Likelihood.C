...
void Likelihood(TString samples){
   ...
   std::vector<TString> Regions={"_nJets_le1"};
   ...
   if (samples.Contains("Data")){
      ...
      for (unsigned int i=0; i<mcversion.size(); i++){
         ...
         // if (prefix=="") prefix="mc";
         for (unsigned int j=0; j<Regions.size(); j++){
          Like *pLikeMC=ComputeLikelihood(samples+"_Default"+Regions[j],InputPath+samples+"MC.list",false,false,true,OutputDirMC,mcversion[i],refDir,ref_hist);
          }
       }
   }
...
//   Float_t EtaBinning[]  = {0, 1.37, 1.52, 1.7, 2.5};
  ...
  if (isData) path="/eos/home-s/shudong/workspace/HHML/Run3/HHARD/gen_ntuples/HHML_2lSC_v2_QmisID_ntuples/"; 
  else{
        //path="/eos/user/d/dparedes/Samples212560/LooseSSML/Skimming/";
        //path="/eos/user/s/shuhui/Samples_HeavyHiggs/common-fw_tag212830/offline/LooseSSML/";
        path="/eos/home-s/shudong/workspace/HHML/Run3/HHARD/gen_ntuples/HHML_2lSC_v2_QmisID_ntuples/";
      //   if (period.Contains("mc16")) path="/eos/user/s/shuhui/Samples_HeavyHiggs/common-fw_tag212830/offline/LooseSSML/"+period+"/ge0j/Skimming/"; 
  }
...
int Like::ComputeRates(std::vector<double> reference){
  ...
  if (reference.size()>0){
   ...
   for (unsigned int i=0; i<NPAR; i++){
      if (i==4 || i==5 || i==6 || i==7) continue; // skip crack region
       step[i]=1e-12;  
   ...
   for (int i=0; i<m_bins_eta; i++){
       for (int j=0; j<m_bins_pt; j++){
           if (i==1) continue; // skip crack region
